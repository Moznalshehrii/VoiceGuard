"""Baseline training loop (Step B). Purpose: validate the full pipeline
end-to-end (data -> model -> loss -> metric), not to hit strong accuracy.
...
Usage (real ASVspoof2019 LA once downloaded):
    python3 -m src.train \
...
from src.data.dataset import AudioSpoofDataset
...
def main(args):
    ...
    train_ds = AudioSpoofDataset(train_df, train=True)
    val_ds = AudioSpoofDataset(val_df, train=False)
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss, train_acc, train_eer = run_epoch(model, train_loader, device, criterion, optimizer)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_path", default="checkpoints/baseline_cnn.pt")
